...
def train_parser():
    ...
    parser.add_argument('--max_epochs', type=int, default=350, help='train for X epochs')
...
def train(params: argparse.Namespace):
    ...
    if params.just_demo:
        ...
        pl.Trainer(accelerator='gpu', devices=gpu_nums, precision=params.precision, limit_test_batches=params.limit_val_batches).test(
            model, dataloaders=classif_data.val_dataloader())
    elif params.just_test:
        pl.Trainer(accelerator='gpu', devices=gpu_nums, precision=params.precision, limit_test_batches=params.limit_test_batches).test(
